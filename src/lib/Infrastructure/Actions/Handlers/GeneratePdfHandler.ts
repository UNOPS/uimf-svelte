...
export class GeneratePdfHandler extends ActionHandler {
	...
	async execute(data: ActionButtonData): Promise<void> {
		...
		if (!params?.DocumentType) {
			console.error('[GeneratePdfHandler] DocumentType is required');
			return;
		}
		...
		if (params.DocumentType === 'sales-order') {
			...
			const response = await this.controller.app.postForm('sales-order-confirmation-pdf', inputFieldValues, null);
			pdfMake
